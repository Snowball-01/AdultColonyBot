...
async def get_xvideos_video_info(link: str) -> dict:
    result = {}
    ...
    try:
        ...
        result.update({"thumbnail": info.get("thumbnails")[-1]["url"]})
        result.update({"description": info.get("description")})

        # Download link logic 🍃
        result.update({"videos": {}})
        for item in info["formats"]:
            if "format_id" in item and len(item["format_id"].split('-')) > 1:
                format_quality = item["format_id"].split('-')[1]
                if format_quality in QUALITIES:
                    result["videos"].update({format_quality: item["url"]})
